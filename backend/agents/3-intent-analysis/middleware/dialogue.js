...
export function inferDialogue(text = "") {
  ...
  const newFacts = !isHypothetical(text) && /(?:라고|라며|라면서|라는데|래요|하라|달라고|하라고|보내래|옮기래|요구했|요구해|연락(?:이|을)\s*(?:왔|받)|전화(?:가|를)\s*(?:왔|받)|알려줬|알려드렸|입력했|보냈|이체했|송금했|설치했|눌렀|클릭했|피해를\s*(?:봤|당)|안\s*보냈|보내지\s*않|신고했|요청했|전화했|(?:생활비|병원비|등록금|용돈).{0,15}(?:보내|송금|주려)|(?:제가|저는).{0,15}(?:보내|송금))/.test(text);
  ...
  if (/그\s*다음|다음에는|이제\s*(?:뭘|무엇)|처리.{0,8}확인|접수.{0,8}확인/.test(text)) return make("progress");
...
export const DIALOGUE_GUIDANCE = {
  ...
  progress:"완료했다고 확인된 조치는 반복시키지 말고, 지금 확인할 다음 조치나 처리 여부 확인 방법에 답하세요.",
